...
const LINK_ORDER = ["listen", "watch", "official", "tickets", "retailer", "sg_search", "taobao_search", "source"];
...
export function useSectionFeed(section, options) {
  ...
  return { feed, data: feed.data, entries, markSeen, live: Boolean(feed.data?.revision) };
}
 ...
 */
export function FeedMedia({ item, media = item.media?.[0], active = true, viewer = false, onGone }) {
  ...
  const video = media && (media.type === "mp4" || (media.type === "hls" && hls));
  // Tumblr's API terms rule out continuous autoplay of GIF sequences: stills inline, play on tap.
  const moving = motionAllowed && !String(item.source ?? "").startsWith("tumblr");
  useEffect(() => {
    const node = ref.current;
    if (!video || viewer || !node || typeof IntersectionObserver === "undefined") return;
  ...
  if (media.type === "gif") return viewer || moving ? picture(media.url) : still("tap to play");
  ...
  return viewer ? (
    <>
      ...
      <a className="text-link" href={item.url} target="_blank" rel="noopener noreferrer">
        Watch on {item.credit?.platform ?? "the source"} <Icon name="diagonal" size={13} />
      </a>
    </>
  ) : (
    still("tap to watch")
...
export function FeedLinks({ item, onOpen, limit = 4 }) {
  ...
  return (
    <div className="feed-links">
      ...
      {taobao && <small className="feed-note">look for 正版 (official) listings, bestie.</small>}
    </div>
  );
}
...
export function FeedFacts({ item }) {
  ...
  if (price) bits.push(`${price}${originalPrice(f) ? ` (${originalPrice(f)})` : ""}`);
  ...
  if (f.eventAt) {
    ...
    const end = f.eventEndAt && sgDate(f.eventEndAt) !== start ? `–${sgDate(f.eventEndAt)}` : "";
    bits.push(`${start}${end}${f.venue ? ` · ${f.venue}` : ""}`);
  }
  if (f.airingAt) bits.push(`${sgDate(f.airingAt, { weekday: "short", day: "numeric", month: "short" })}, ${sgTime(f.airingAt)}`);
  ...
  return (
    <div className="feed-actions">
      ...
      >
        ...
        {kept ? "kept ♡" : keep.data?.preview ? "preview only" : keep.isPending ? "keeping…" : "keep"}
      </button>
      <button
        type="button"
        className="quiet-button"
        ...
        onClick={() => {
          ...
          if (window.confirm("Hide this from your feeds for good?")) hide.mutate(item.id);
        }}
      >
      ...
      {(keep.error || hide.error) && (
        <small role="status" className="feed-note">
          {keep.error ? "couldn’t keep that one. try again in a bit." : "couldn’t hide that one. try again in a bit."}
        </small>
      )}
    </div>
  );
...
export function FeedViewer({ entry, onClose, onSeen }) {
  ...
  return (
    <Modal title={item.blurb?.headline || item.title} onClose={onClose} className="gallery-dialog feed-viewer">
      {song ? (
        <button
        ...
        <FeedMedia item={item} viewer onGone={(id) => (reportGone(id), onClose())} />
      )}
      <p>{item.blurb?.text}</p>
      <FeedFacts item={item} />
      {note && (
        <aside className="note-slip feed-companion">
          <strong className="feed-card__headline">{note.blurb?.headline}</strong>
          <p>{note.blurb?.text}</p>
...
export function FeedCard({ entry, label, onSeen, showMedia = true, children }) {
  ...
  return (
    <article ref={ref} className="note-slip feed-card" data-seen={entry.seen ? "" : undefined}>
      ...
      {item.blurb?.headline && <strong className="feed-card__headline">{item.blurb.headline}</strong>}
      <p>{item.blurb?.text}</p>
      <FeedFacts item={item} />
      {children}
      ...
      {open && <FeedViewer entry={entry} onClose={() => setOpen(false)} onSeen={seen} />}
    </article>
  );
}
...
export function FeedPhoto({ entry, index, title, caption, onSeen }) {
  ...
  const seen = () => onSeen?.(entry.ids);
  useSeenOnView(ref, seen);
  if (!item.media?.length) return <FeedCard entry={entry} label={title} onSeen={onSeen} />;
  return (
    <div ref={ref} className="feed-photo">
      <button
        type="button"
...
export function FeedSleeve({ entry, onSeen }) {
  ...
  const image = cover?.poster ?? (cover?.type === "image" ? cover.url : null) ?? song?.thumbnail;
  const inside = (
    <>
      ...
      <span className="record-sleeve__label">{entry.seen ? "SEEN EARLIER" : "TODAY’S DROP"}</span>
    </>
  );
  return (
    <div ref={ref} className="feed-sleeve">
      {song ? (
        ...
        <a className="record-sleeve" href={item.url} target="_blank" rel="noopener noreferrer" onClick={seen}>
          {inside}
        </a>
      )}
...
export function PersonalPhoto({ personal, index }) {
  ...
  return (
    <div className="feed-photo">
      ...
      {open && (
        <Modal title={personal.character} onClose={() => setOpen(false)} className="gallery-dialog">
...
export function EpisodeBanner({ episode }) {
  ...
  let line;
  if (!airing) line = `ep ${episode.episode} reactions are in ♡ the first slides are all about it.`;
  else if (left > 0) {
  ...
  return (
    <aside className="note-slip feed-episode">
      <span className="micro-label">{airing ? "EPISODE DAY" : "THE MORNING AFTER"}</span>
      <p>{line}</p>
    </aside>
  );
}
    ...
    <>
      <p className="collection-label feed-progress">
        ...
        <span className="collection-label__detail">
          {counts.new} new{counts.seenEarlier ? ` · ${counts.seenEarlier} seen earlier` : ""}
        </span>
      </p>
      {data.closing && (
...
/** The top merch item for a section, as one line with a link to the whole shelf. */
export function MerchLine({ entry, onSeen }) {
  const ref = useRef(null);
  useSeenOnView(ref, () => entry && onSeen?.(entry.ids), { enabled: Boolean(entry) });
  if (!entry) return null;
  const item = entry.primary;
  const shop = (item.facts?.links ?? []).find((l) => ["retailer", "official", "sg_search"].includes(l.kind));
  return (
    <aside ref={ref} className="note-slip feed-merch-line">
      <span className="micro-label">A LITTLE SOMETHING FOR THE SHELF</span>
      <p>
        <strong>{item.title}</strong> {item.blurb?.text ? `· ${item.blurb.text}` : ""}
      </p>
      <FeedFacts item={item} />
      <div className="note-slip__source">
        {shop ? (
          <a href={shop.url} target="_blank" rel="noopener noreferrer" onClick={() => onSeen?.(entry.ids)}>
            {shop.label} ↗
          </a>
        ) : (
...
export function MemeOfTheDay() {
  ...
  return (
    <section className="editorial-section feed-meme" aria-labelledby="meme-title">
      <div className="feed-meme__inner">
        ...
        <aside className="note-slip">
          <span className="micro-label" id="meme-title">
            MEME OF THE DAY{entry.seen ? " · SEEN EARLIER" : ""}
          </span>
          ...
          {entries.length > 1 && (
            ...
            >
              one more <Icon name="arrow" size={14} />
            </button>
          )}
...
export function EventsStrip({ limit = 6 }) {
  ...
  const list = (events.data?.events ?? [])
    ...
    .sort((a, b) => Number(b.hers) - Number(a.hers) || String(a.startsOn ?? "9999").localeCompare(String(b.startsOn ?? "9999")))
    .slice(0, limit);
  if (!list.length) return null;
  return (
    <div className="feed-events" aria-label="Upcoming events">
      <p className="collection-label">
        <span />
        EVENTS COMING UP
        <span className="collection-label__detail">dates from official pages</span>
      </p>
      <ul>
        {list.map((event) => {
          ...
          return (
            <li key={event.id} className="note-slip">
              <span className="micro-label">
                {event.hers ? "UR PICK · " : ""}
                {event.tier === "sg" ? "SINGAPORE" : (event.city ?? "REGIONAL").toUpperCase()}
                {event.tbc ? " · TBC" : ""}
              </span>
              ...
              <div className="note-slip__source">
                ...
                {countdownLabel(days) && <span className="handwritten">{countdownLabel(days)}</span>}
              </div>
            </li>
          );
        })}
