...
- `checkItem` runs source rules, moderation, then still-image vision. Missing/malformed/unavailable checks stay pending. A source whose entry sets `visionPolicy: "platform"` (YouTube, and `vocadb-songs`, whose media is a YouTube embed) is trusted with the picture by owner decision (2026-09-17): nothing is downloaded or classified, the playback gate and the free text moderation still run, its `evidence.scope` is `embed-provenance+platform-checks`, and section eligibility falls back to the item's own tags. Those items never consume a section's paid check quota. Non-still media stays `moving_review_required`; a poster does not approve the clip/GIF. Current adapters cannot publish moving media automatically.
