...
| **YouTube Data API** · `youtube-music`, `youtube-tutorials`, `youtube-apothecary` (enabled, fetch-b), `youtube-memes` (enabled, fetch-a) | [Developer policies](https://developers.google.com/youtube/terms/developer-policies), [API docs](https://developers.google.com/youtube/v3/docs) | `YOUTUBE_API_KEY` | 19:20:36 | **verified live 2026-09-17** from this machine (the owner put a readable key in `.env.local`): all 32 channel IDs resolve and pass `channelTitleMatches`, and `youtube-memes`, `youtube-tutorials` and `youtube-apothecary` each returned real items (probe records in `.data/feed-probes/`). Two verified edit channels (MaomaoEdit 115k subs ~7 uploads/week, Mayuri Edits 33k ~2.6/week) give the meme carousel a floor and returned six Apothecary Shorts on the first run; both carry search_result provenance because an edit channel is not the author. Two Apothecary queries returned trailer reuploads and were replaced (≈1,200 quota units/day — 11 `search.list` calls at 100 units each plus ≈60 list units — and ≈1 unit per published YouTube item at recheck) | song, video, clip, tutorial, cosplay, meme · `embed_provenance` (embeddable, SG region, age restriction, provenance recorded) | link_only · recheck before publish · data expires ≤30 d | VocaDB songs, news |
| **YouTube live broadcasts** · `youtube-lives` (enabled, fetch-b) | [Developer policies](https://developers.google.com/youtube/terms/developer-policies), [API docs](https://developers.google.com/youtube/v3/docs/search/list) | `YOUTUBE_API_KEY` | 21:11 (2026-09-17) | **verified live 2026-09-17**: three channels probed, one real upcoming stream collected (Elira Pendora, scheduled 23:00 UTC the same evening) with `official_channel` provenance and `facts.eventAt` set. All 13 VTuber channel IDs resolved from their @handles the same day. A live broadcast reports `uploadStatus: "uploaded"`, so `playbackFrom` accepts that value for live items only | event (`facts.eventAt`) · `embed_provenance` | link_only · recheck before publish · data expires ≤30 d | the music section's live slot, countdown |
| **Reddit Data API** · `reddit-fandom` (enabled, fetch-a) | [Data API terms](https://redditinc.com/policies/data-api-terms), [API docs](https://www.reddit.com/dev/api/) | `REDDIT_CLIENT_ID`, `REDDIT_CLIENT_SECRET` (+ optional `REDDIT_USERNAME` for the User-Agent) | — | **not yet called live**: Reddit answers 403 without a token and the owner's app was still being created, so the eight subreddit names are unconfirmed. Mock-tested end to end (token, eight pages, placement, deletion recheck). Free tier 100 requests/minute; this makes ~10 a day, paced 1.5 s | image, news, cosplay, meme, merch · `still_only` (static JPEG/PNG/WebP on Reddit's own hosts, hot-linked) | link_only · honour deletions ≤24 h via `/api/info` | every section |
