...
| **YouTube Data API** · `youtube-music`, `youtube-tutorials`, `youtube-apothecary` (enabled, fetch-b), `youtube-memes` (enabled, fetch-a) | [Developer policies](https://developers.google.com/youtube/terms/developer-policies), [API docs](https://developers.google.com/youtube/v3/docs) | `YOUTUBE_API_KEY` | 19:20:36 | `not_configured_locally`; **no live call made** (mock-tested; ≈1,200 quota units/day — 11 `search.list` calls at 100 units each plus ≈60 list units — and ≈1 unit per published YouTube item at recheck) | song, video, clip, tutorial, cosplay, meme · `embed_provenance` (embeddable, SG region, age restriction, provenance recorded) | link_only · recheck before publish · data expires ≤30 d | VocaDB songs, news |
